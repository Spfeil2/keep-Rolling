...
app.get('/getObstructionById/:id', async (req, res) => {
  ...
  try{
      const sql_result = await client.query(`SELECT * FROM meldungen WHERE id = ${id}`)
...
app.get('/filterObstructions', async (req, res) => {
  ...
  try {	 
    if (providedDays && !providedTypes && !providedTimestamp) {
      ...
      const offset = 1;
         const placeholders = selectedTypes.map(function(name,i) { 
            return '$'+(i+offset); 
       }).join(',');
...
app.get('/getAllObstructions', async (req, res) => {
  ...
  try{
      const sql_result = await client.query("SELECT name, latitude, longitude, type, date, id FROM meldungen")
      res.status(200).send(sql_result)
  } catch (e) {
...
app.delete('/delete/:id', async (req, res) => {
  const id = parseInt(req.params.id)
  try{
      const sql_result = await client.query(`DELETE FROM meldungen WHERE id = ${id}`)
